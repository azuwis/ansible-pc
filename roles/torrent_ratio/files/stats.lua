...
if not args.raw then
  ngx.say('info_hash                                report_uploaded uploaded downloaded announce incomplete')
end
for _, info_hash in ipairs(keys) do
  ...
  if not args.raw then
    ...
    info = string.format('%-15s %-8s %-10s %-8s %-10s', format(report_uploaded), format(uploaded), format(downloaded), math.floor((current_epoch - epoch) / 60), incomplete)
  end
